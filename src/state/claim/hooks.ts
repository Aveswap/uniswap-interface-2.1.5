...
function fetchClaimMapping(): Promise<ClaimAddressMapping> {
  return (
    ...
    (FETCH_CLAIM_MAPPING_PROMISE = fetch(
      `https://raw.githubusercontent.com/Aveswap/mrkl-drop-data-chunks/final/chunks/mapping.json`
    )
      .then((res) => res.json())
      .catch((error) => {
...
function fetchClaimFile(key: string): Promise<{ [address: string]: UserClaimData }> {
  return (
    ...
    (FETCH_CLAIM_FILE_PROMISES[key] = fetch(
      `https://raw.githubusercontent.com/Aveswap/mrkl-drop-data-chunks/final/chunks/${key}.json`
    )
      .then((res) => res.json())
      .catch((error) => {
...
// check if user is in blob and has not yet claimed AVE
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unclaimedAmount?.toSignificant(4)} AVE`,
